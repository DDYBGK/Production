...
def load_model(model_type=None):
    if model_type == "pointnet2":
        experiment_dir = 'log/sem_seg/pointnet2_sem_seg/'  # 模型A路径
    elif model_type == "pointnet":
        experiment_dir = 'log/sem_seg/pointnet_sem_seg/'
    ...
    checkpoint = torch.load(str(experiment_dir) + '/checkpoints/best_model.pth', weights_only=False,
                            map_location=device)  # Load with weights_only=False
...
def inference(classifier, input_path, visual_dir):
    ...
    TEST_DATASET_WHOLE_SCENE = ScannetDatasetWholeScene(os.path.dirname(input_path), split='test',
                                                        test_area=int(test_area_index), block_points=NUM_POINT)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_path', type=str, default='./data/Area_5_conferenceRoom_1.npy',
    ...
    mIoU = inference(classfier, args.input_path, args.output_path)